...
class SaveNextInvetBidResultData:
    def __init__(self):
        self.input_next_invest_bid_result_sql_path = read_conf.ReadConf().get_options("path_next_invest_bid", "next_invest_ratio_result_sql_path")

    def get_result_data(self, is_success):
        try:
            os_v = hive_command_method.hive_command('-f', self.input_next_invest_bid_result_sql_path)
            if os_v == 0:
                is_success.append(1)
            else:
                is_success.append(0)
