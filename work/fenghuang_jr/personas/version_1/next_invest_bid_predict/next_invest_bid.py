...
class NextInvestBid:
    def __init__(self):
        ...
        self.result_data_rate_path = read_conf.ReadConf().get_options("path_next_invest_bid", "next_invest_ratio_result_data_path")
        self.result_data_deadline_path = read_conf.ReadConf().get_options("path_next_invest_bid", "next_invest_deadline_result_data_path")
        self.result_data_parent_path = read_conf.ReadConf().get_options("path_next_invest_bid", "next_invest_result_data_parent_path")
    ...
    def implement_process(self):
        ...
        root_logger.debug('NextInvestBid start logger...')
        logger = logging.getLogger('personas.next_invest_bid')
        """ 得到训练集 """
        ...
        if flag == 1:
            ...
            logger.error("error !")
            send_mail.send_mail(self.mail_tolist, self.mail_host, self.mail_user, self.mail_password, self.mail_postfix, "personas_NextInvestBid_error", "NextInvestBid is error！")
        for v in self.is_success:
            if v == 0:
                flag = 0
                break
        ...
        application_method.move_file(self.result_data_parent_path, self.result_data_rate_path)
        application_method.move_file(self.result_data_parent_path, self.result_data_deadline_path)
