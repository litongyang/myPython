...
class UserRelationInvest:
    def __init__(self):
        ...
        self.result_data_bid_path = read_conf.ReadConf().get_options("path_user_relation_invest", "input_user_relation_invest_bid_result_data_path")
        self.result_data_user_path = read_conf.ReadConf().get_options("path_user_relation_invest", "input_user_relation_invest_user_result_data_path")
        self.result_data_parent_path = read_conf.ReadConf().get_options("path_user_relation_invest", "input_user_relation_invest_result_data_parent_path")
        self.is_success = []
    ...
    def implement_process(self):
        ...
        root_logger.debug('UserRelationInvest start logger...')
        logger = logging.getLogger('personas.user_relation_invest')
        """ 得到训练集 """
        ...
        if flag == 1:
            ...
            logger.error("error!")
            send_mail.send_mail(self.mail_tolist, self.mail_host, self.mail_user, self.mail_password, self.mail_postfix, "personas_UserRelationInvest_error", "UserRelationInvest is error!")
        for v in self.is_success:
            if v == 0:
                flag = 0
                break
