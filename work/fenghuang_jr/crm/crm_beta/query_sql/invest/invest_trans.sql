...
(
	select
		...
		sum(case when i.CREDITASSIGNID is null then 1 else 0 end) as no_debt_swap_cnt,
		sum(case when i.CREDITASSIGNID is null then 0 else 1 end) as debt_swap_cnt,
	...
	LEFT OUTER JOIN (
		...
		AND inv.CREDITASSIGNID IS NULL
		AND inv.STATUS IN (
			'FROZEN',
			'FINISHED',
			'SETTLED',
