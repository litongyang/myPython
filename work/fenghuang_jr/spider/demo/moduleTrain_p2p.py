...
id = []
X = []
fi = open('pre_data.txt', 'w')

for line in open('train_set_p2p.txt'):
    line_one = line.split()
...
#  save and load modelclf = KMeans(n_clusters=3, random_state=random_state)
# clf = KMeans(n_clusters=5, random_state=random_state)
# clf.fit(X)
# from sklearn.externals import joblib
# joblib.dump(clf, 'model\p2p\model_fund.pkl')
# clf1 = joblib.load('model\p2p\model_fund.pkl')
# y = clf1.predict(X)
# print y
