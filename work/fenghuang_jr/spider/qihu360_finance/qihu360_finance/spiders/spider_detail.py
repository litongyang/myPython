...
class SpiderDetailSpider(scrapy.Spider):
    ...
    #     'https://www.nicaifu.com/dq/detail?code=DDF3H6RXQLxqw9CV8fPIB8ro8Njd5LvF',
    #     # 'https://www.nicaifu.com/dc/detail?code=DC2803',
    #     # 'https://www.nicaifu.com/ins/detail?product_code=BULY',
    # ]
    # start_urls = ['https://www.nicaifu.com/ins/detail?product_code=BULY']
    start_urls = get_url_class.GetUrl().get_news_time_url_dict()
    ...
    def parse(self, response):
        try:
            ...
            title_template1 = root.xpath('//h3/text()')
            title_template2 = root.xpath('//h3/a/text()')
            ...
            if len(title_template1) > 0:
                if len(title_template1) >= 1:
                    title_nodes = title_template1[len(title_template1)-1]
                else:
                    title_nodes = title_template1[0]
            elif len(title_template2) > 0:
                title_nodes = title_template2[0]
            ...
            # print "================="
            item['url'] = response.url  # 产品详情url
            item['title'] = title_nodes  # 产品标题
            item['open_date'] = open_date  # 开标日期
            ...
            item['ts'] = self.time_satmp  # 爬取时间
            yield item

        except Exception, e:
            if response.url.find('reglogin?&returnUrl'):  # 需要登录
                pass
            else:
