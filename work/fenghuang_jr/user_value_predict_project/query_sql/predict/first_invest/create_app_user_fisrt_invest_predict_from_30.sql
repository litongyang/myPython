--drop table app.app_user_predict_first_invest_predict_set_from_30_full;
--create table if not exists app.app_user_predict_first_invest_predict_set_from_30_full
--COMMENT '首投30天后预测未来投资的预测集'
--ROW FORMAT DELIMITED
--FIELDS TERMINATED BY '\t'
--LINES TERMINATED BY '\n'
--as
insert overwrite table app.app_user_predict_first_invest_predict_set_from_30
select
	a.user_id,
    b.age_generation,
    b.gender_code,
...
from
  (
  	...
  	where
  		dt=edw_day_amount(-1)
  		and to_date(first_invest_tm)<edw_day_amount(-31) -- 第一次预测
  		--and to_date(first_invest_tm)=edw_day_amount(-31)
...
on
  a.user_id = b.user_id
left outer join
  app.app_user_predict_first_invest_account_feature_2017_full c
on
  a.user_id = c.user_id
left outer join
