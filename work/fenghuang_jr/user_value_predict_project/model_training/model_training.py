...
class ModelTraining:
    ...
    def save_result_file(self, user_id, predict_type, pre_label, ts, save_result_path, success):
        ...
        """
        logging.config.fileConfig('../logger.conf')
        logger = logging.getLogger('user_predict.save_result')
        try:
            ...
            exception = Exception, e
            error_info = str(exception) + "--------->>" + "save_result is Exception !"
            logger.error(error_info)
            success.append(0)
    ...
    def model_report(self, true_label, pre_label, report_file, success):
        ...
        try:
            ...
            fw.write('\n')
            report = metrics.classification_report(np.asarray(true_label, dtype='int'), np.asarray(pre_label, dtype='int'))
            fw.write(str(report))
            success.append(1)
        except Exception, e:
            exception = Exception, e
