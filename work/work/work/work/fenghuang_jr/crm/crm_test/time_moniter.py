...
class TimeMoniter:
    ...
    def get_diff(self, read_conf_class, file_dir_process_class, log_path):
        try:
            ...
            print self.diff
            file_dir_process_class.wirte_file(log_path, "Runing time difference: %s" % self.diff)
        except Exception, e:
    ...
    def is_all_fun(self, read_conf_class):
        ...
        try:
            # print read_conf_class.get_options('is_all', 'is_overdue_days')
            if self.diff < int(read_conf_class.get_options('is_all', 'is_overdue_days')):
                print "%s process is  append  input" % self.date
            else:
                print "%s process is  all  input" % self.date
                self.is_all = 1
        except Exception, e:
            error_info = Exception, ": ", e
            self.error_log.write_log(error_info)

    def write_cycle_all(self, read_conf_class, file_dir_process_class, log_path):
        try:
            if self.is_all == 1:
                read_conf_class.set_options("cycle_all", "content", "all")
                file_dir_process_class.wirte_file(log_path, "Refresh Type : all")
            else:
                read_conf_class.set_options("cycle_all", "content", "append")
                file_dir_process_class.wirte_file(log_path, "Refresh Type : append")
        except Exception, e:
            error_info = Exception, ": ", e
            self.error_log.write_log(error_info)
