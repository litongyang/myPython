...
class PARAMETER_ANALYSIS():
    ...
    def Parameter_ROE(self):
        try:
            ...
            # print y
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='净资产收益率'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.ROE = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
            ...
            for j in range(0, self.year_len):
                temp += '\'' + str(self.ROE[j]) + '\'' + ','
            temp = temp[0:-1]  # 去除最后一个逗号
            # x = "insert into %s values(%s)" % (self.fileName, temp)
            # print x
            self.cur.execute("insert into %s values(%s)" % (self.fileName, temp))
        except Exception, e:
            self.fl.write(self.data_file)
            self.fl.write('\n')
            print Exception, e
    ...
    def Parameter_Gross_margin(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='销售毛利率'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.Gross_margin = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
    ...
    def Parameter_Sales_net_profit_rate(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='净利润'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.net_profit = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
        ...
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='营业收入' OR SUBJECT='营业总收入'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.business_income = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i] != 0:
    ...
    def Parameter_Rate_of_return_on_total_assets(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='利润总额'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.total_profit = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i] == 0:
                        self.total_profit.append(-1)
                    else:
        ...
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='应付利息'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.interest_expense = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
        ...
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='资产总计'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.total_assets_avg = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
    ...
    def Parameter_Main_business_profit_rate(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='营业利润'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.business_profit = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
    ...
    def Parameter_Inventory_turnover_days(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='营业成本'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.cost_in_business = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
        ...
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='存货'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.remainder = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    self.remainder.append(results[0][i] * 10000)
                    self.remainder_avg.append((results[0][i] + results[0][i + 1]) / 2 * 10000)
            if len(self.remainder_avg) == 0:
                self.remainder_avg = [-1 for i in range(self.year_len)]
            for i in range(0, len(self.remainder_avg)):  # 排除分母为0的case
                if float(self.remainder_avg[i]) == 0:
                    self.remainder_avg[i] = -1
            temp1 = [self.cost_in_business[i] / self.remainder_avg[i] for i in range(0, len(self.cost_in_business))]
            for i in range(0, len(temp1)):   # 排除分母为0的case
                if float(temp1[i]) == 0:
                    temp1[i] = -1
    ...
    def Parameter_Accounts_receivable_turnover_days(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='应收账款'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.accounts_receivable_avg = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    self.accounts_receivable_avg.append((results[0][i] + results[0][i + 1]) / 2 * 10000)
            for i in range(0, len(self.remainder_avg)):  # 排除分母为0的case
                if float(self.accounts_receivable_avg[i]) == 0:
                    self.accounts_receivable_avg[i] = -1
    ...
    def Parameter_Quick_ratio(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='流动资产合计'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.liquid_assets = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
        ...
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='流动负债合计'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.current_liabilities = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i] == 0:
                        self.current_liabilities.append(-1)
                    else:
    ...
    def Parameter_Cash_ratio(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='货币资金'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.cash = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
        ...
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='交易性金融资产'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.trading_financial_assets = [0 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
    ...
    def Parameter_Cash_liabilities_rate(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='经营现金流量净额'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.Net_operating_cash_flow = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
        ...
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='负债合计'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.total_liabilities = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i] == 0:
                        self.total_liabilities.append(-1)
                    else:
    ...
    def Parameter_Rate_of_assets_and_liabilities(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='资产负债比率'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.Rate_of_assets_and_liabilities = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
    ...
    def Parameter_Equity_ratio(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='股东权益合计'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.shareholders_equity = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i] == 0:
                        self.shareholders_equity.append(-1)
                    else:
    ...
    def Parameter_Debt_to_tangible_assets_ratio(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='无形资产'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.intangible_assets = [0 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
            ...
            for i in range(0, len(self.Debt_to_tangible_assets_ratio)):
                if self.Debt_to_tangible_assets_ratio[i] == 0:
                    self.Debt_to_tangible_assets_ratio[i] = -1
    ...
    def Parameter_Sales_revenue_growth_rate(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='营业收入'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.Sales_revenue_growth_rate = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i + 1] == 0:
                        self.Sales_revenue_growth_rate.append(-1)
                    else:
    ...
    def Parameter_Pre_tax_profit_growth_rate(self):
        try:
            ...
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.Pre_tax_profit_growth_rate = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i + 1] == 0:
                        self.Pre_tax_profit_growth_rate.append(-1)
                    else:
    ...
    def Parameter_Total_assets_growth_rate(self):
        try:
            ...
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.Total_assets_growth_rate = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i + 1] == 0:
                        self.Total_assets_growth_rate.append(-1)
                    else:
    ...
    def Parameter_Asset_growth_rate(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='每股净资产'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.Asset_growth_rate = [-1 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
                    if results[0][i + 1] == 0:
                        self.Asset_growth_rate.append(-1)
                    else:
    ...
    def Parameter_Net_profit_growth_rate(self):
        try:
            self.cur.execute('set names \'utf8\'')
            count = self.cur.execute("SELECT * FROM %s WHERE SUBJECT='净利润同比增长率'" % self.data_file)
            results = self.cur.fetchmany(count)
            if len(results) == 0:
                self.Net_profit_growth_rate = [0 for i in range(self.year_len)]
            else:
                for i in range(3, 3 + self.year_len):
...
if __name__ == '__main__':
    ...
    # for Parameter.Company_code in range(3, 4):
        ...
        print Parameter.fileName + " of inserting is successful!"
